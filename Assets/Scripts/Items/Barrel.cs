...
public class Barrel : SceneItem,IBarrelSound
{
    ...
    public override void ApplyDamage(int damage, Vector2 pos,float force)
    {
        ...
        if (Health == 1)
        {
            ...
            _particleObject = _particleManager.PlayDetachedParticle(smoke,_smokeTransform.position,timeToExp,transform,Destroing);
    ...
    public override void Destroing()
    {
        ...
        {
            if (_loader.damagableObjects.TryGetValue(colMass[i].gameObject, out _distractable))
            { 
