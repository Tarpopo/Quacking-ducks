...
public class RocketBullet : BulletComponent
{
    ...
    {
        _particleSystem = ItemsSpawner.SpawnObject(_transform.position, ObjectId.RocketParticle, true)
    ...
    private void Explotion()
    {
        ...
        {
            // if (ItemsSpawner.damagableObjects.TryGetValue(colMass[i].gameObject, out _item))
            // {
            //     _item.ApplyExplosionDamage(_damage, _transform.position, _force, _damageRadius);
            // }

            if (colMass[i].TryGetComponent<IDamagable>(out var damageable) == false) continue;
            damageable.ApplyExplosionDamage(_damage, _transform.position, _force, _damageRadius);
        }
    }
}